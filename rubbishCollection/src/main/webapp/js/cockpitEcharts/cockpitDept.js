...
$(document).ready(function(){
	...
	function createCircularData(data,map){
    	...
    	initCircularEchart(chart,map);
	}
	
	//加载圆饼图
	function initCircularEchart(datas,map) {
	    //获取当前父元素的宽度
		var width = $(".sencend-con").find("#two-cockpit-detail-con").width();
		//获取绑定总数
		...
		$.ajax({
			...
			success : function(data){
				if(null != data && "" != data){
					...
					var datas = data.list;
					$("#PV_TOTAL_DAY").text(data.PV_TOTAL_TODAY);//使用用户  
					$("#UV_TOTAL_DAY").text(data.GROWTH_DIFFERENCE);//今日同比昨天增加
	...
	function showCockpit(flag,title){
		...
		var sendDialog = parent.ns.modalDialog({
			...
			height : 500,
			resizable : true,
			url :url+'/cockpit/CockpitController/showCockpit.do?flag='+ flag +'&title='+encodeURI(encodeURI(title))+'&dept_code='+dept_code
		});
	}
