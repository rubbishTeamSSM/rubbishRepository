...
//加载固定页面的操作菜单
function initFixeAction(menu_code, button_code){
	var data = {'menu_code':menu_code, 'button_code' : button_code};
	$.ajax({
		type : 'post',
       	url : url + '/businessConsole/role/getRoleMenuButton.do',
       	...
       	async : false,
       	success : function(result) {
       	     var buttonArray = result;
       		 $(".buttonFixeAction a").each(function(){
       		      var buttonName = $(this).attr("buttonName");
       		      for(var i=0;i<buttonArray.length;i++){
       		          if(buttonName==buttonArray[i].button_name){
       		             var id = $(this).attr("id");
       		             $("#"+id).show();
       		             $("#"+id).attr("buttonCode",buttonArray[i].button_code);
       		             break;
       		          }
       		      }
       		});
    ...
    duplicateCheck: {
		...
		data.MODI_VALUE = MODI_VALUE;
			var checkR = $.ajax({
					...
					cache : false,
					type : 'post',
					url : url+'/comboboxMgr/duplicateCheck.do',
...
function getNowTimeNoSeconds(){
	...
	//当前分
	var minutes = myDate.getMinutes();
	return year + '-' + p(month) + '-' + p(date) + ' ' + p(hours) + ':' + p(minutes);
}
//数组去重
...
function editCombo(datas,inpId,value){
    	$.ajax({
          	type : 'post',
          	url : basePath + '/comboboxMgr/getDictionaryList.do',
          	dataType : 'json',
          	data:{'data':JSON.stringify(datas)},
          	success : function(data) {
	   			$("#"+inpId).combobox({
					...
					//editable:false,
					panelHeight:'100',
					validType : 'comboboxCheck['+JSON.stringify(datas)+']',
					onLoadSuccess:function(){
						$("#"+inpId).combobox('setValue',value);
					}
	  			});
       		},
          	error: function(){
...
//文件下载方法
function downloadFile(wjlj){
        var filelj = wjlj.split("/");
        var pageName = filelj[filelj.length-1];
        //window.location.href=url+'/download.tool?wjlj='+wjlj;
        $.ajax({
		type : 'post',
		url : url+'/businessConsole/ftp/getDownloadWj.do',
		data :{'wjlj':wjlj},
		dataType : 'json',
		success : function(result) {
		     //console.info(result);
		     if (result.success) {
				 window.location.href = '${ct}/businessConsole/ftp/downloadTask.do?filepath='+ result.msg+'&pageName='+encodeURI(encodeURI(pageName));
			 }else{
				 parent.$.messager.alert('错误', '下载失败!', 'error');
			 }
		},
   ...
   function downloadFileAndName(wjlj,name){
        var filelj = wjlj.split("/");
        var pageName = name;
        $.ajax({
		type : 'post',
 ...
 * @return {Number} 
 */
function getDayNo(startTime,endTime){
    ...
    var iDays = parseInt((strDateS - strDateE)  / 1000 / 60 / 60 /24);
    return iDays;
}
//时间转化，获取指定时间的字符串，以-分割
function getDateString(nowDate){
    var nowTime = nowDate.getFullYear()+'-'+(Number(nowDate.getMonth())+1);
    var tempday = nowDate.getDate();
    if(10>tempday){
        nowTime = nowTime +'-0'+ tempday;
    }else{
        nowTime = nowTime +'-'+ tempday;
    }
    return nowTime;
}
...
div.datagrid-cell-c1-PROGRESS{position:relative;}
</style>
